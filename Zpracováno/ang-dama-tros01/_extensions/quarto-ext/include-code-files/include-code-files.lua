--- include-code-files.lua – filter to include code from source files
---
--- Copyright: © 2020 Bruno BEAUFILS
--- License:   MIT – see LICENSE file for details

--- Dedent a line
local function dedent(line, n)
  return line:sub(1, n):gsub(" ", "") .. line:sub(n + 1)
end

--- Find snippet start and end.
--
--  Use this to populate startline and endline.
--  This should work like pandocs snippet functionality: https://github.com/owickstrom/pandoc-include-code/tree/master
local function snippet(cb, fh)
  local name = cb.attributes.snippet or cb.attributes.region
  if not name then
    return
  end

  -- Cannot capture enum: http://lua-users.org/wiki/PatternsTutorial
  local comment
  local comment_stop = ""
  if
      string.match(cb.attributes.include, ".py$")
      or string.match(cb.attributes.include, ".jl$")
      or string.match(cb.attributes.include, ".r$")
      or string.match(cb.attributes.include, ".yaml$")
      or string.match(cb.attributes.include, ".yml$")
  then
    comment = "#"
  elseif string.match(cb.attributes.include, ".o?js$") or string.match(cb.attributes.include, ".css$") or string.match(cb.attributes.include, ".ts$") then
    comment = "//"
  elseif string.match(cb.attributes.include, ".lua$") then
    comment = "--"
  elseif string.match(cb.attributes.include, ".html$") or string.match(cb.attributes.include, ".xml$") then
    comment = "<!%-%-"
    comment_stop = " *%-%->"
  else
    -- If not known assume that it is something one or two long and not alphanumeric.
    comment = "%W%W?"
  end

  -- Escape comment for lua pattern if needed (simple approach)
  -- The original code did not escape, assuming safe chars for regex or relying on %W%W?

  -- Original snippet patterns
  local p_start_snippet = string.format("^ *%s start snippet %s%s", comment, name, comment_stop)
  local p_stop_snippet = string.format("^ *%s end snippet %s%s", comment, name, comment_stop)

  -- New region patterns (Quarto style)
  local p_start_region = string.format("^ *%s| region: %s%s", comment, name, comment_stop)
  local p_stop_region = string.format("^ *%s| endregion: %s%s", comment, name, comment_stop)

  local start, stop = nil, nil

  -- Cannot use pairs.
  local line_no = 1
  for line in fh:lines() do
    if start == nil then
      if string.match(line, p_start_snippet) or string.match(line, p_start_region) then
        start = line_no + 1
      end
    elseif stop == nil then
      if string.match(line, p_stop_snippet) or string.match(line, p_stop_region) then
        stop = line_no - 1
      end
    else
      break
    end
    line_no = line_no + 1
  end

  -- Reset so nothing is broken later on.
  fh:seek("set")

  -- If start and stop not found, just continue
  if start == nil or stop == nil then
    return nil
  end

  cb.attributes.startLine = tostring(start)
  cb.attributes.endLine = tostring(stop)
end

--- Filter function for code blocks
local function transclude(cb)
  if cb.attributes.include then
    local content = ""
    local fh = io.open(cb.attributes.include)
    if not fh then
      -- Fallback for project-root execution: try 2..simulace/src/
      local alt_path = "2..simulace/src/" .. cb.attributes.include
      fh = io.open(alt_path)
      if fh then
        -- Found in alternative path
      end
    end
    if not fh then
      io.stderr:write("Cannot open file " .. cb.attributes.include .. " | Skipping includes\n")
    else
      local number = 1
      local start = 1

      -- change hyphenated attributes to PascalCase
      for i, pascal in pairs({ "startLine", "endLine" }) do
        local hyphen = pascal:gsub("%u", "-%0"):lower()
        if cb.attributes[hyphen] then
          cb.attributes[pascal] = cb.attributes[hyphen]
          cb.attributes[hyphen] = nil
        end
      end

      -- Overwrite startLine and stopLine with the snippet if any.
      snippet(cb, fh)

      if cb.attributes.startLine then
        cb.attributes.startFrom = cb.attributes.startLine
        start = tonumber(cb.attributes.startLine)
      end

      -- Determine dedent level if requested
      local dedent_amount = 0
      if cb.attributes.dedent then
        if cb.attributes.dedent == "true" then
          -- Auto-detect minimum indentation
          local min_indent = 1000
          local lines = {}
          for line in fh:lines("L") do
            table.insert(lines, line)
            if line:match("%S") then
              local indent = line:match("^ *"):len()
              if indent < min_indent then min_indent = indent end
            end
          end
          dedent_amount = min_indent

          -- Reset file handle for main loop (inefficient but safe)
          -- Actually better to just iterate the lines table

          -- RE-IMPLEMENTATION: Read all lines first to support auto-dedent
        else
          dedent_amount = tonumber(cb.attributes.dedent) or 0
        end
      end

      -- Reset if we read for auto-dedent, or just read normally
      fh:seek("set")

      for line in fh:lines("L") do
        if number >= start then
          if not cb.attributes.endLine or number <= tonumber(cb.attributes.endLine) then
            if dedent_amount > 0 then
              line = dedent(line, dedent_amount)
            end
            content = content .. line
          end
        end
        number = number + 1
      end

      fh:close()
    end

    -- remove key-value pair for used keys
    cb.attributes.include = nil
    cb.attributes.startLine = nil
    cb.attributes.endLine = nil
    cb.attributes.dedent = nil

    -- return final code block
    return pandoc.CodeBlock(content, cb.attr)
  end
end

return {
  { CodeBlock = transclude },
}
