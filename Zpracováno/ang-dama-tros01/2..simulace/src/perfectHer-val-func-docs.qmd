...
# Robustni nalezeni project root (pro staged i source render)
if basename(cur_doc_dir) == "staging_temp"
    # Jsme v staging_temp (primo v root projektu)
    project_root = dirname(cur_doc_dir)
else
    # Jsme v ang-dama-tros01/2..simulace/src/ (3 urovne hluboko)
    project_root = joinpath(cur_doc_dir, "..", "..", "..")
end

sim_base_path = joinpath(project_root, "simulation_data")
if !isdir(sim_base_path)
    # Zkusime fallback do stareho umisteni (kdyby nahodou)
    sim_base_path = joinpath(project_root, "out", "simulation_outputs")
end
...
if !isdir(sim_base_path)
    ...
    open(joinpath(sim_base_path, "run_dummy", "pruběh_simulace.txt"), "w") do f
